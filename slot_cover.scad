...
module slot_cover_side(h,up,down,cut_up,offs,rounded=[0,0])
{
	offs_=3.5;
	dd=0.3;
	linear_extrude(h)
	offset(delta=offs)
		polygon (polyRound([
			  [0,-up,0]
			, [offs_+0.2,-up,rounded[0]]
			, [offs_+0.2,0.84,0]
			, [offs_-0.36,1.63,0]
			, [offs_-0.2,2.385,0]
			, [offs_-0.55,3+down,0]
			, [offs_-1.33,3+down,0]
			, [offs_-1.33,0.9-cut_up,rounded[1]]
			, [0,0.9-cut_up,0]
		],20));
}

module slot_cover(h,up=0,down=0,cut_up=0,rounded=[0,0])
{
