...
module slot_cut(height,grooves=[0,1,2,3],offs=0.2)
{
	difference()
	{
...
module pulley_cut(pulley_type,op=0,angle,screw,up,nut_type="hex",out=[0,0],nut_offset=2.6,report=false)
{
	offs=[1,1];
	...
	if (op==0)
	{
		...
		pulley_cut(pulley_type=pulley_type,op=2,angle=angle,screw=screw,up=up,out=out,nut_offset=nut_offset,nut_type=nut_type);
	}
	if (op==1)
	{
		dd=3+1;
	...
	if (op==2)
	{
		...
		rotate ([180,0,0])
		{
			if (nut_type=="hex")
			{
				...
				translate ([0,0,screw-nut_offset])
				{
					m3_nut_inner();
